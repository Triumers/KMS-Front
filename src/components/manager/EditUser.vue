<template>
    <div class="container">
      ...
      <form @submit.prevent="updateMember">
        ...
        <input type="file" id="profileImg" @change="handleProfileImgChange" accept="image/jpeg" />
        </div>
        <div class="form-group">
...
onMounted(async () => {
  try {
    ...
    const response = await axios.get(`http://triumers-back.ap-northeast-2.elasticbeanstalk.com/api/endpoint/employee/find/id/${memberId}`, {
      headers: {
        Authorization: token,
      },
    });
...
async function updateMember() {
  try {
    ...
    formData.append('teamId', selectedTeam.value.id);
    formData.append('positionId', selectedPosition.value.id);
    formData.append('rankId', selectedRank.value.id);

    const response = await axios.post('http://triumers-back.ap-northeast-2.elasticbeanstalk.com/api/endpoint/manager/edit/info', formData, {
      headers: {
        Authorization: token,
